        ...
        <tbody>
          {maintenanceList.map((item) => (
            <tr key={item._id}>
              ...
              <td>{item.status}</td>
              <td>
                <button style={{ color: "teal" }} onClick={() => handleEdit(item)}> <i className="fa-solid fa-pen-to-square"></i></button>
                <button style={{ color: "rgb(251, 90, 90)" }} onClick={() => handleDelete(item._id)}>  <i className="fa-solid fa-trash"></i></button>
              </td>
            </tr>
          ))}
        </tbody>
