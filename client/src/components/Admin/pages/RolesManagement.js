...
const RolesManagement = () => {
  ...
  const fetchRoles = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://clinic-management-7k6q.onrender.com/api/admin/roles");
  ...
  const handleUpdateRole = async (role) => {
    try {
      const res = await fetch(
        `https://clinic-management-7k6q.onrender.com/api/admin/roles/${role._id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(role),
  ...
  const handleDeleteRole = async (id) => {
    try {
      const res = await fetch(`https://clinic-management-7k6q.onrender.com/api/admin/roles/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        fetchRoles();
