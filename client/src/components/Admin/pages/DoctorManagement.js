...
const DoctorManagement = () => {
  ...
  const fetchDoctors = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://clinic-management-7k6q.onrender.com/api/admin/doctors");
  ...
  const handleUpdateDoctor = async (doctor) => {
    try {
      const res = await fetch(
        `https://clinic-management-7k6q.onrender.com/api/admin/doctors/${doctor._id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(doctor),
  ...
  const handleDeleteDoctor = async (id) => {
    try {
      const res = await fetch(`https://clinic-management-7k6q.onrender.com/api/admin/doctors/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        fetchDoctors();
