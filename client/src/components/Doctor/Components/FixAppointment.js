    ...
    const fetchAppointments = async () => {
      try {
        const response = await axios.get(
          "https://clinic-management-7k6q.onrender.com/api/doctor/appointmentconfirmation"
        );
  ...
  const handleAction = async (id, action, updatedInfo = null) => {
    try {
      ...
      const response = await axios.put(
        `https://clinic-management-7k6q.onrender.com/api/doctor/appointmentconfirmation/${id}`,
        payload
      );
